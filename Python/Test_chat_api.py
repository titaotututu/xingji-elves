...
if __name__ == "__main__":
    # Step 1: 创建两个聊天会话
    user_id = 7690 # 假设用户 ID 为 1
    session_id_1 = create_chat_session(user_id)
    session_id_2 = create_chat_session(user_id)

    if session_id_1 and session_id_2:
        # Step 2: 在第一个会话中发送消息
        send_chat_message(session_id_1, "Hello from session 1")
        send_chat_message(session_id_1, "How are you?")

        # Step 3: 在第二个会话中发送消息
        send_chat_message(session_id_2, "Hello from session 2")
        send_chat_message(session_id_2, "What's the weather like?")
